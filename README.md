...
| `WG_DEFAULT_ADDRESS`          | `10.8.0.x`        | `10.6.0.x`, `10.8.0.x/16`     | Clients IP address range. Append `/CIDR` to use a larger subnet (e.g. `/16` for ~65000 clients, `/8` for ~16M). Default without CIDR suffix is `/24` (253 clients).                                                      |
